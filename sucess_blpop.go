...
func handleConnection(conn net.Conn) {
	...
	for scanner.Scan() {
		...
		case command == "lpush":
			...
			queue[key] = ed
			fmt.Println("Lpush value : %s", ed.items)
		...
		case command == "blpop":
			...
			if !exists || len(list.items) == 0 {
				...
				go func() {
					...
					case data := <-data:
						set = data
						fmt.Println("value from go routine %s", data)
						ch <- data
					}
				}()
				go func() {
				...
				fmt.Printf("for the BLPOP: %v \n", list.items)

				//}
				item := list.items[0]
				fmt.Printf("value from the Blpop %s \n", item)
			...
			case item := <-ch:
				...
				queue[key] = list
				fmt.Println("value from channel %s", item)
				fmt.Fprintln(conn, item)
			}
		// lrange mylist 0 -1
		case command == "lrange":
